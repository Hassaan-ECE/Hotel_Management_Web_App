...
- Front desk reservations opens at `/hotels/<hotelId>/front-desk/reservations`; booking board is the default, the Table / Booking board toggle stays side-by-side on mobile, the table remains available, table search/filter/sort work, and check-in/check-out actions use the guarded status route.
- Front desk booking board defaults to a 7-day exclusive range starting today; date range changes auto-update the URL after valid start/end values, empty room rows are hidden by default, longer ranges use one shared compressed timeline without horizontal dragging, reservation bars align to room/date spans, bars show centered guest names only, and clipped bars have squared/thicker clipped-side edges.
- Front desk mobile density check: topbar shows the current front-desk context, page-title blocks are absent, subnav/action buttons stay side-by-side, room cells show centered room numbers only, Status and `Show empty rooms` share a compact controls row, and there is no `Apply dates` button.
