...
import { finishHousekeepingRoom } from "@/lib/hotel-service";
...
export async function POST(request: Request, { params }: { params: Promise<{ hotelId: string }> }) {
  try {
    ...
    const { session } = await requireHotelSession(hotelId, ["owner", "manager", "housekeeping", "housekeeping-supervisor"]);
    const input = await parseJson(request, roomActionSchema);
    await finishHousekeepingRoom(hotelId, session, input.roomId);
